...
const player = { x: canvas.width / 2, y: canvas.height - 60, w: 40, h: 40 };
let bullets = [];
let enemies = [];
let lastBullet = 0;
let lastEnemy = 0;
...
function reset(){
  bullets=[];enemies=[];score=0;scoreEl.textContent=score;gameOver=false;
...
function update(dt,now){
  ...
  enemies.forEach((e,ei)=>{
    bullets.forEach((b,bi)=>{
      if(rectHit(b,e)){enemies.splice(ei,1);bullets.splice(bi,1);score++;scoreEl.textContent=score;}
    });
    if(rectHit(e,player)){gameOver=true;messageEl.classList.remove('hidden');}
  });
}
